...
Future<bool> checkRootAccess() async {
  try {
    final result = await _methodChannel.invokeMethod<bool>('hasRoot');
    return result ?? false;
  } catch (e) {
    _logger.warning('checkRootAccess failed: $e');
    return false;
  }
}

/// 请求 root 权限（会触发 Magisk/SuperSU 弹窗）
Future<bool> requestRootAccess() async {
  try {
    final result = await _methodChannel.invokeMethod<bool>('requestRoot');
    return result ?? false;
  } catch (e) {
    _logger.warning('requestRootAccess failed: $e');
    return false;
  }
}

/// 设置 SELinux 强制模式
/// [permissive] = true  -> 宽容模式（允许访问 /data）
/// [permissive] = false -> 强制模式
Future<bool> setSelinuxMode({bool permissive = true}) async {
  try {
    final result = await _methodChannel.invokeMethod<bool>('setEnforce', {'permissive': permissive});
    return result ?? false;
  } catch (e) {
    _logger.warning('setSelinuxMode failed: $e');
    return false;
  }
}

/// 以 root 身份执行命令
Future<String> execRootCommand(String cmd) async {
  try {
    final result = await _methodChannel.invokeMethod<String>('execRootCmd', {'cmd': cmd});
    return result ?? '';
  } catch (e) {
    _logger.warning('execRootCommand failed: $e');
    return 'ERROR: $e';
  }
}
