      ...
      builder: (context, vm) {
        ...
        return ResponsiveListView(
          ...
          children: [
            ...
            if (vm.advanced && checkPlatform([TargetPlatform.android]))
              _RootAccessSection(),
            _SettingsSection(
...
class _RootAccessSection extends StatefulWidget {
  @override
  State<_RootAccessSection> createState() => _RootAccessSectionState();
}

class _RootAccessSectionState extends State<_RootAccessSection> {
  bool _hasRoot = false;
  ...
  Future<void> _checkRootStatus() async {
    final hasRoot = await android_channel.checkRootAccess();
    if (mounted) {
      setState(() {
        _hasRoot = hasRoot;
        _loading = false;
  ...
  Future<void> _checkSelinuxStatus() async {
    final output = await android_channel.execRootCommand('getenforce');
    if (mounted) {
      setState(() {
        _selinuxDisabled = output.toLowerCase().contains('permissive');
      });
    }
  }
  ...
  Future<void> _requestRootAccess() async {
    ...
    // 先请求 root 权限（会弹出 Magisk 授权框）
    final granted = await android_channel.requestRootAccess();

    if (!granted) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Root 授权失败，请尝试在 Magisk 中授权 LocalSend')),
        );
      }
    ...
    // Root 授权成功后，禁用 SELinux（宽容模式），允许访问 /data
    final selinuxOk = await android_channel.setSelinuxMode(permissive: true);
    if (!selinuxOk) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Root 已授权，但 SELinux 设置失败（部分设备不支持）')),
        );
      }
    } else {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Root 权限获取成功！/data 目录已可访问')),
        );
      }
    }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text('Root Access', style: Theme.of(context).textTheme.titleMedium),
                  const SizedBox(width: 8),
                  Tooltip(
                    message: '获取超级用户权限以访问 /data 目录',
              ...
              Text(
                '需要 Root 权限才能访问受系统保护的 /data 目录。'
                '授权后 LocalSend 将能够浏览并发送 /data 目录下的文件。',
              ...
              if (_loading)
                ...
                Row(
                  children: [
                    Icon(
                      _hasRoot ? Icons.check_circle : Icons.cancel,
                      color: _hasRoot ? Colors.green : Colors.red,
                      size: 18,
                    ),
                    ...
                    Text(
                      _hasRoot
                          ? _selinuxDisabled
                              ? 'Root 已授权 | SELinux 已关闭 | /data 可访问'
                              : 'Root 已授权 | SELinux 未关闭 | /data 可能受限'
                          : 'Root 未授权',
                      style: TextStyle(
                        color: _hasRoot ? Colors.green : Colors.red,
                        fontSize: 13,
