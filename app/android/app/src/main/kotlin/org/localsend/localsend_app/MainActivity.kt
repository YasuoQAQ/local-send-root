...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(
            ...
            when (call.method) {
                ...
                "requestRoot" -> {
                    val granted = requestRootAccess()
                    result.success(granted)
                }

                "setEnforce" -> {
                    val permissive = call.argument<Boolean>("permissive") ?: true
                    val success = setEnforceMode(permissive)
                    result.success(success)
                }

                "execRootCmd" -> {
                    val cmd = call.argument<String>("cmd") ?: ""
                    val output = execRootCommand(cmd)
    ...
    /// 以 root 身份执行命令
    private fun execRootCommand(cmd: String): String {
        if (!checkRootAccess()) return "ERROR: no root access"
        return try {
    ...
    /// 设置 SELinux 强制模式
    /// permissive=true -> setenforce 0 (宽容模式，允许访问 /data)
    /// permissive=false -> setenforce 1 (强制模式)
    private fun setEnforceMode(permissive: Boolean): Boolean {
        if (!checkRootAccess()) return false
        return try {
            val enforceCmd = if (permissive) "0" else "1"
            val process = Runtime.getRuntime().exec(arrayOf("su", "-c", "setenforce $enforceCmd"))
            process.waitFor()
            process.exitValue() == 0
        } catch (e: Exception) {
            false
